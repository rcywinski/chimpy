...
var filesGlobValue = './lib/**/*.spec.js';
var filesGlobFiles = ['./lib/fileA.spec.js', './lib/someDir/fileB.spec.js'];
var emptyTestsDirPath = '';
describe('mocha-wrapper', function () {
  beforeEach(function () {
    ...
    this.path = td.replace('path', td.object({
      join: emptyFn,
      resolve: emptyFn
    }));
    this.glob = td.replace('glob', td.object({
      sync: emptyFn
    }));
    process.env.mochaConfig = JSON.stringify({
      tags: ''
    });
    process.argv = [];
  });
  ...
  describe('add files', function () {
    it('adds files inside testDir', function () {
      process.argv = [];
      process.env['chimp.path'] = testDir; // mock --path tests dir files
      ...
      var MochaWrapper = require('./mocha-wrapper.js').MochaWrapper;
      ...
      td.verify(this.mocha.addFile(), {
        times: 3,
        ignoreExtraArgs: true
      }); // verify only called three times
    });
    it('adds files specified by --files options when --path is empty dir', function () {
      process.argv = [];
      process.env['chimp.path'] = emptyTestsDirPath;
      process.env['chimp.files'] = filesGlobValue; // mock empty --path tests dir files

      td.when(this.path.join('mocha-helper.js')).thenReturn('mocha-helper.js');
      td.when(this.path.resolve(__dirname, 'mocha-helper.js')).thenReturn('mocha-helper.js');
      td.when(this.path.join(emptyTestsDirPath, '**')).thenReturn(emptyTestsDirPath);
      td.when(this.glob.sync(emptyTestsDirPath)).thenReturn([]); // mock --files flag files
    ...
    it('adds files specified by --files option and files in --path when both contain files', function () {
      ...
      process.env['chimp.files'] = filesGlobValue; // mock mocha-helper

      td.when(this.path.join('mocha-helper.js')).thenReturn('mocha-helper.js');
      td.when(this.path.resolve(__dirname, 'mocha-helper.js')).thenReturn('mocha-helper.js'); // mock --files flag files

      td.when(this.glob.sync(filesGlobValue)).thenReturn(filesGlobFiles); // mock --path test dir files
    ...
    it('adds files specified on cli directly', function () {
      process.argv = ['/usr/local/bin/node', 'chimp', '--mocha', 'test/fileA.js', 'test/fileB.js', 'test/fileC.js', 'fileD.js']; // mock mocha-helper
